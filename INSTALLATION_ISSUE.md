...
Test the basic plugin flow (standard permission request + get contacts) on a fresh device before implementing the limited authorization feature. Add native-side logging to verify methods are being called.

Use the stable published version:

## Workaround

```bash

npm install @capacitor-community/contacts@^7.0.0Use the stable published version until the PR is merged:
